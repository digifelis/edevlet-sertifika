...
<div class="card card-default">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form class="form-horizontal" action="{{route('superadmin.users.edit', ['id'=>$user->id])}}" method="post">
                @csrf
                <div class="card-body">
                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-6">
                      <div class="form-group">
                        ...
                        <option value="none">seçiniz</option>
                          <option value="active" {{$user->userStatus == 'active'? 'selected':''}}>aktif</option>
                          <option value="passive" {{$user->userStatus == 'passive'? 'selected':''}}>pasif</option>
                  ...
                  <select class="custom-select rounded-0" id="exampleSelectRounded0" name="userInstitution">
                    ...
                    <option value="{{$kurum->id}}" {{$user->userInstitution == $kurum->id? 'selected':''}}>{{$kurum->kurumAdi}}</option>
                    @endforeach
                  </select>
                </div>
